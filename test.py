    ...
    def analyze_runway_capacity(self, airport: str) -> Dict:
        ...
        if len(airport_data) == 0:
            ...
            return {
                ...
                "avg_offpeak_utilization": 0.0,
                "peak_hours": config["peak_hours"],
                "most_congested_hour": 12,  # Default to noon
                "least_congested_hour": 3,  # Default to 3 AM
        ...
        return {
            "max_capacity": config["runways"] * config["capacity_per_hour"],
            "avg_peak_utilization": peak_hours["Utilization"].mean() if len(peak_hours) > 0 else 0.0,
            "avg_offpeak_utilization": off_peak_hours["Utilization"].mean() if len(off_peak_hours) > 0 else 0.0,
            "peak_hours": config["peak_hours"],
            "most_congested_hour": airport_data.loc[airport_data["CongestionLevel"].idxmax(), "Hour"] if len(
                airport_data) > 0 else 12,
            "least_congested_hour": airport_data.loc[airport_data["CongestionLevel"].idxmin(), "Hour"] if len(
                airport_data) > 0 else 3,
            "runway_count": config["runways"]
        }
...
def map_query(query: str, sbert_model, col_keys, col_emb, op_keys, op_emb, df):
    ...
    for token in ["will", "predict", "probability", "chance", "will it be delayed", "delayed?", "will my flight be",
...
def main():
    ...
    st.title("Enhanced Flight Schedule Optimizer")
    ...
    if uploaded:
        ...
        if os.path.exists(args.features):
            ...
            st.warning("No data found. Upload CSV or run pipeline to generate outputs/features.parquet")
            st.stop()
    ...
    if "PrevDelayByTail" not in df.columns:
        if "TailNumber" in df.columns and ("ArrivalDelay" in df.columns or "DepDelay" in df.columns):
            df = df.sort_values(["TailNumber", "Month", "DayOfWeek", "DepartureTime"] if all(
                col in df.columns for col in ["TailNumber", "Month", "DayOfWeek", "DepartureTime"]) else ["TailNumber"])
    ...
    user_q = st.text_input(
        "Query",
        placeholder="Try:'What's the delay probability for DEL-BOM at 8 AM?'",
    ...
    if user_q:
        ...
        if mapping["intent_optimize"] and optimizer is not None:
            ...
            tab1, tab2, tab3 = st.tabs(["Recommendations", "Analysis", "Heatmap"])
            ...
            with tab2:
                ...
                with col2:
                    ...
                    st.write("**Peak Hours:**",
                             ", ".join([f"{h:02d}:00" for h in capacity_analysis.get("peak_hours", [])]))
                    st.metric("Most Congested Hour",
                              f"{capacity_analysis.get('most_congested_hour', 'N/A'):02d}:00" if isinstance(
                                  capacity_analysis.get('most_congested_hour'), int) else "N/A")
                    st.metric("Least Congested Hour",
                              f"{capacity_analysis.get('least_congested_hour', 'N/A'):02d}:00" if isinstance(
                                  capacity_analysis.get('least_congested_hour'), int) else "N/A")
            ...
            st.warning(
                "⚠️ Optimization features not available due to data compatibility issues. Please check your dataset format.")
            ...
            with st.form("prediction_form"):
                ...
                with col1:
                    ...
                    origin = st.selectbox("Origin Airport", ["BOM", "DEL", "BLR", "MAA", "HYD", "CCU"],
                                          index=0 if not mapping["filters"]["origin"] else ["BOM", "DEL", "BLR", "MAA",
                                                                                            "HYD", "CCU"].index(
                                              mapping["filters"]["origin"]) if mapping["filters"]["origin"] in ["BOM",
                                                                                                                "DEL",
                                                                                                                "BLR",
                                                                                                                "MAA",
                                                                                                                "HYD",
                ...
                with col2:
                    ...
                    hour = st.slider("Departure Hour", 0, 23, value=int(
                        (mapping["filters"]["hour_range"][0] if mapping["filters"]["hour_range"] else 8)))
            ...
            if submitted:
                ...
                # Run prediction
                ensemble_result = ensemble_predict(artifacts["rf_pipe"], artifacts["model_b"], artifacts["meta"],
                                                   input_features)
